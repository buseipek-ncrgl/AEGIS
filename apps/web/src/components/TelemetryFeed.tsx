      ...
      <div className="bg-[#090e1a]/95 px-4 py-2.5 border-b border-cyan-500/30 flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xs font-black text-cyan-300 tracking-widest uppercase">
            CANLI TELEMETRİ VE KİNEMATİK ANOMALİ AKIŞI (SIGNALR WEBSOCKET)
          </h3>
        </div>
        ...
        <div className="flex items-center space-x-3">
          {telemetryLogs.length > 0 && (
            <button
              ...
              title="Görev Sonrası Raporu (AAR) CSV İndir"
            >
              <Download className="w-3.5 h-3.5 text-emerald-400" /> RAPOR İNDİR (CSV)
            </button>
          )}
          ...
          <span className="text-[10px] font-mono text-emerald-400 font-bold bg-emerald-950/80 border border-emerald-500/40 px-2 py-0.5 rounded flex items-center gap-1">
            <Radio className="w-3 h-3 animate-pulse text-emerald-400" /> AKIŞ AKTİF
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 font-mono text-xs space-y-1.5 bg-[#050913]/90">
        {telemetryLogs.length === 0 ? (
          <div className="text-center py-12 text-slate-500 text-xs">
            Canlı telemetri paketleri bekleniyor... Akış için Python Simülatörünü çalıştırın.
          </div>
        ) : (
            ...
            >
              <div className="flex items-center space-x-2 flex-wrap">
                ...
                <span className="text-slate-400 text-[11px]">
                  ENLEM: <strong className="text-slate-200">{item.telemetry.latitude.toFixed(4)}</strong> BOYLAM: <strong className="text-slate-200">{item.telemetry.longitude.toFixed(4)}</strong>
                </span>
              ...
              <div className="flex items-center space-x-4 text-slate-400 text-[11px]">
                <span>
                  İRTİFA: <strong className="text-emerald-400 font-bold">{item.telemetry.altitude}m</strong>
                </span>
                <span>
                  HIZ: <strong className="text-amber-400 font-bold">{item.telemetry.speed}km/h</strong>
                </span>
                <span className={item.telemetry.batteryPercentage < 20 ? "text-rose-400 font-bold animate-pulse" : "text-cyan-300 font-bold"}>
                  BATARYA: %{item.telemetry.batteryPercentage}
                </span>
              </div>
            </div>
          ))
